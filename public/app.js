$(document).ready(function() {
    ...
    $('#buscar').click(function() {
        if ($("#checkPersonalizada")[0].checked){
            ...
            var urls = `http://localhost:7070/ciudad/${$("#ciudad").val()}/tipo/${$("#tipo").val()}/desde/${valores[0]}/hasta/${valores[1]}`;
        } else {
            var urls = "http://localhost:7070/search";
        }
        $.ajax({
            url: urls,
            type: 'get',
        ...
        objArr.forEach(function(key, idx)
        {
            html += `<div class="card horizontal">
                    <div class="card-image">
                        <img src="http://localhost:7070/img/home.jpg">
                    </div>
    ...
    function init(){
        $.ajax({
            url: 'http://localhost:7070/filteroptions',
            type: 'get',
            dataType: 'json'
        })
        .done(function(data) {
